...
class myFilter : public vtkRotationalExtrusionFilter
{
public:
    static myFilter* New()
    {
        ...
        vtkInformationVector** inputVector, vtkInformationVector* outputVector) override
    {
        ...
        for (i = 1; i <= this->Resolution; i++)
        {
            ...
            for (ptId = 0; ptId < numPts; ptId++)
            {
                ...
                double newr = std::sqrt(newd0 * newd0 + newd1 * newd1 + newd2 * newd2);
...
int main()
{
...

    {
        ...
        filter->SetDeltaRadius(1.0);

        vtkNew<vtkPolyDataNormals> normals;
        normals->SetInputConnection(filter->GetOutputPort());
        normals->SetFeatureAngle(60);

        vtkns::renderPolydata(ren, normals->GetOutputPort());
    }
